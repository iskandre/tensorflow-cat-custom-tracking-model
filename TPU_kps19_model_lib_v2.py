...
def eager_train_step(detection_model,
                     ...
                     num_replicas=1.0, vars_to_fine_tune = []):
  ...
  # trainable_variables = vars_to_fine_tune
  trainable_variables = detection_model.trainable_variables
    
  # trainable_variables = [x for x in detection_model.trainable_variables if x.name in vars_to_fine_tune]
    
  # print('box scale weights')
  # tf.print(detection_model._prediction_head_dict['box/scale'][0].layers[0].weights[0][0,0,:3,:3])
  # print('keypoints heatmap weights')
  # tf.print(detection_model._prediction_head_dict['human_pose/keypoint/heatmap'][0].layers[0].weights[0][0,0,:3,:3])
  ...
  with summary_writer.as_default():
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        # _ensure_model_is_built(detection_model, train_input, unpad_groundtruth_tensors)
        ...
        box_offset_layers = detection_model._prediction_head_dict.get('box/offset')
        cats_id_coco = 16

        for vars_ind in range(0,2):
            required_var_names = ['model/_prediction_head_dict/box.Soffset/0/layer_with_weights-%s/bias/.ATTRIBUTES/VARIABLE_VALUE'%vars_ind,
            'model/_prediction_head_dict/box.Soffset/0/layer_with_weights-%s/kernel/.ATTRIBUTES/VARIABLE_VALUE'%vars_ind]
            bias = reader.get_tensor(required_var_names[0])
            kernel = reader.get_tensor(required_var_names[1])
            ind = vars_ind
            if vars_ind == 1:
                ind = 2
            # kernel
            assert(box_offset_layers[0].layers[ind].trainable_variables[0].shape == kernel.shape)
            tf.compat.v1.assign(detection_model._prediction_head_dict['box/offset'][0].layers[ind].trainable_variables[0], kernel)
            # bias
            assert(box_offset_layers[0].layers[ind].trainable_variables[1].shape == bias.shape)
            tf.compat.v1.assign(detection_model._prediction_head_dict['box/offset'][0].layers[ind].trainable_variables[1], bias)
            
            
        box_scale_layers = detection_model._prediction_head_dict.get('box/scale')
        for vars_ind in range(0,2):
            required_var_names = ['model/_prediction_head_dict/box.Sscale/0/layer_with_weights-%s/bias/.ATTRIBUTES/VARIABLE_VALUE'%vars_ind,
            'model/_prediction_head_dict/box.Sscale/0/layer_with_weights-%s/kernel/.ATTRIBUTES/VARIABLE_VALUE'%vars_ind]
            bias = reader.get_tensor(required_var_names[0])
            kernel = reader.get_tensor(required_var_names[1])
            ind = vars_ind
            if vars_ind == 1:
                ind = 2
            # kernel
            assert(box_scale_layers[0].layers[ind].trainable_variables[0].shape == kernel.shape)
            tf.compat.v1.assign(detection_model._prediction_head_dict['box/scale'][0].layers[ind].trainable_variables[0], kernel)
            # bias
            assert(box_scale_layers[0].layers[ind].trainable_variables[1].shape == bias.shape)
            tf.compat.v1.assign(detection_model._prediction_head_dict['box/scale'][0].layers[ind].trainable_variables[1], bias)
            
        object_center_layers = detection_model._prediction_head_dict.get('object_center')
        for vars_ind in range(0,2):
            required_var_names = ['model/_prediction_head_dict/object_center/0/layer_with_weights-%s/bias/.ATTRIBUTES/VARIABLE_VALUE'%vars_ind,
            'model/_prediction_head_dict/object_center/0/layer_with_weights-%s/kernel/.ATTRIBUTES/VARIABLE_VALUE'%vars_ind]
            bias = reader.get_tensor(required_var_names[0])
            kernel = reader.get_tensor(required_var_names[1])
            if vars_ind == 1:
                # if loading weights from already pre-trained model here, then there is no cats_id_coco, it's 0 in that axis
                kernel = tf.expand_dims(kernel[:,:,:,cats_id_coco],axis=-1)
                bias = tf.expand_dims(bias[vars_ind],axis=-1)
            ind = vars_ind
            if vars_ind == 1:
                ind = 2
            # kernel
            assert(object_center_layers[0].layers[ind].trainable_variables[0].shape == kernel.shape)
            tf.compat.v1.assign(detection_model._prediction_head_dict['object_center'][0].layers[ind].trainable_variables[0], kernel)
            # bias
            assert(object_center_layers[0].layers[ind].trainable_variables[1].shape == bias.shape)
            tf.compat.v1.assign(detection_model._prediction_head_dict['object_center'][0].layers[ind].trainable_variables[1], bias)
        
        print('all weights are restored')

        ###### My custom part #####
        # variables can be freezed, however with TPU training we have enough resources not to free them and let them fine-tune
        # vars_to_fine_tune = [x.name for x in detection_model._feature_extractor.trainable_variables]
        ...
        img_data = tf.io.gfile.GFile('my__cat0.jpg', 'rb').read()
        ...
        print('cat preds')
        print(detections['detection_scores'][0,:5])
        
        print(detection_model._prediction_head_dict['object_center'][0].layers[0].weights[0][0,0,:3,:3])
        ...
        def train_step_fn(features, labels):
          ...
          losses_dict = eager_train_step(
              ...
              vars_to_fine_tune = vars_to_fine_tune)
